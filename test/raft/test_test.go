...
func TestReElection2A(t *testing.T) {
	...
	println("con1-------", leader1)
	cfg.connect(leader1)
	leader2 := cfg.checkOneLeader()

	// if there's no quorum, no leader should
	// be elected.
	println("disc2-------", leader2)
	cfg.disconnect(leader2)
	println("disc3 nonleader-------", (leader2+1)%servers)
	cfg.disconnect((leader2 + 1) % servers)
	time.Sleep(2 * RaftElectionTimeout)
	cfg.checkNoLeader()

	// if a quorum arises, it should elect a leader.
	println("con2-------", (leader2+1)%servers)
	cfg.connect((leader2 + 1) % servers)
	cfg.checkOneLeader()
	println("con3-------", leader2)
...
func TestRejoin2B(t *testing.T) {
	...
	println("here")
	cfg.one(103, 2, true)
	println("leader 1 dic and send 103")
	cfg.printLog()
	cfg.printLog2()
