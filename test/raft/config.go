	...
	for time.Since(t0).Seconds() < 10 {
		...
		if index != -1 {
			...
			for time.Since(t1).Seconds() < 2 {
				...
				//println(nd)
				//fmt.Println("DIFFFFFF", cmd1, " ", cmd, " ", nd, " ", index)
...
func (cfg *config) printLog2() {
	...
	for j := 0; j < cfg.n; j++ {
		if cfg.connected[j] {
			//fmt.Print(cfg.rafts[j].me, " term: ", cfg.rafts[j].Term, " ")
			for i := 0; i < len(cfg.rafts[j].Log); i++ {
				fmt.Print(cfg.rafts[j].Log[i].Command, "(", cfg.rafts[j].Log[i].Term, ") ")
			}
			println(" ")
		}
	}
...
func (cfg *config) end() {
	...
	if cfg.t.Failed() == false {
		...
		fmt.Printf("  ... Passed --")
		fmt.Printf("%4.1f %dserver, %4d rpc call, %7d byte transmite %4d\n", t, npeers, nrpc, nbytes, ncmds)
	}
}
